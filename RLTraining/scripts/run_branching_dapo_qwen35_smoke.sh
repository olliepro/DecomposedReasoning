...
export REPETITION_CHECKING_ENABLED="${REPETITION_CHECKING_ENABLED:-True}"
export USE_FULL_STOP_STRINGS="${USE_FULL_STOP_STRINGS:-False}"
export INITIAL_ASSISTANT_PREFIX="${INITIAL_ASSISTANT_PREFIX:-}"
...
case "${ROLLOUT_MODE}" in
  ...
  epsilon_greedy_off_policy)
    ...
    set_default SELECTOR_MODE embed_diverse_topk_random
    set_default BRANCH_PROB 0.0
    set_default BRANCH_FANOUT "${OFF_POLICY_BRANCH_FANOUT:-2}"
    set_default MAX_BRANCH_POINTS_PER_ROLLOUT "${OFF_POLICY_MAX_BRANCH_POINTS_PER_ROLLOUT:-1}"
    set_default OFF_POLICY_MIN_CANDIDATES 3
    set_default OFF_POLICY_MAX_CANDIDATES 10
    set_default NUM_CANDIDATES "${OFF_POLICY_MAX_CANDIDATES}"
    set_default EPSILON_GREEDY_PROB "${OFF_POLICY_EPSILON_GREEDY_PROB:-0.1}"
    ;;
  *)
...
export ROLLOUT_MAX_NUM_BATCHED_TOKENS="${ROLLOUT_MAX_NUM_BATCHED_TOKENS:-420000}"
