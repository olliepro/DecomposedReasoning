...
class BranchingRolloutSettings:
    ...
    Args:
        ...
        repetition_checking_enabled: Whether repeated exec/steer block detection
            can force-close a rollout.
        use_full_stop_strings: Whether vLLM decode requests should stop on
            full control tags instead of legacy partial prefixes.
        initial_assistant_prefix: Optional assistant prefill text appended after
            the chat template generation prompt.
        ...
        persistent_log_interval_steps: Persist completed SQLite tree logs every
            N training steps. Current and failed steps remain on disk.
    ...
    update_mode: str = "all"
    persistent_log_interval_steps: int = 10
    ...
    def __post_init__(self) -> None:
        ...
        assert (
            self.persistent_log_interval_steps > 0
        ), "persistent_log_interval_steps must be positive."
        assert (
        ...
        if self.rollout_mode == "epsilon_greedy_off_policy":
            assert self.branch_fanout <= self.off_policy_min_candidates, (
    ...
    def from_config(cls, config: Any) -> "BranchingRolloutSettings":
        ...
        return cls(
            ...
            trigger_steer_enabled=cls._parse_bool(
                value=payload.get("trigger_steer_enabled", True)
            ),
            repetition_checking_enabled=cls._parse_bool(
                value=payload.get("repetition_checking_enabled", True)
            ),
            use_full_stop_strings=cls._parse_bool(
                value=payload.get("use_full_stop_strings", False)
            ),
            initial_assistant_prefix=cls._parse_text(
                value=payload.get(
                    "initial_assistant_prefix",
                    os.environ.get("INITIAL_ASSISTANT_PREFIX", ""),
                )
            ),
            ...
            persistent_log_interval_steps=int(
                payload.get("persistent_log_interval_steps", 10)
            ),
        )
    ...
    def _parse_bool(*, value: object) -> bool:
        """Parse bool-like Hydra values without treating "False" as truthy."""
    ...
    def _parse_text(*, value: object) -> str:
        """Parse Hydra string values while allowing shell-safe `\\n` escapes."""
    ...
    def should_persist_step_logs(self, *, global_step: int) -> bool:
        """Return whether completed branching logs should remain on disk."""
        ...
        if global_step <= 0:
            return True
        return global_step % self.persistent_log_interval_steps == 0
