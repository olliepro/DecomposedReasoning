...
def build_branch_token_offsets(
    *, tree: BranchTree, leaf: LeafRollout, prompt_token_count: int
    ...
    for parent_node_id, child_node_id in zip(path_node_ids[:-1], path_node_ids[1:]):
        ...
        if child_node.branch_token_offset is not None:
            offsets.append(
                min(max(int(child_node.branch_token_offset), 0), response_token_count)
            )
            continue
...
def build_reward_scores(
    *,
    ...
    off_policy_token_spans: tuple[tuple[int, int], ...] = (),
    ...
    Args:
        ...
        off_policy_token_spans: Response-token spans inserted by the rollout engine
            and excluded from actor updates.
    ...
    if off_policy_token_spans:
        reward_scores["off_policy_token_spans"] = [
            [span_start, span_end] for span_start, span_end in off_policy_token_spans
        ]
